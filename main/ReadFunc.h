...
public:
	...
	static void ReadlineInterval(double& tb_epsilon, double& x_start, double& x_end, double& x_del, double& a) {
		tb_epsilon = Read::GetPositivNum("Введите точность (для корректной работы программы ее значение должно быть больше 0):"s);
		do {
			x_start = Read::GetNum("Введите x_start:"s);
			x_end = Read::GetNum("Введите x_end : "s);
			do {
				x_del = Read::GetNum("Введите шаг интервала (для корректной работы программы оно не должно быть равно нулю):"s);
			} while (x_del == 0);
		...
		a = Read::GetNum("Введите значение параметра a:"s);
	}
private:
